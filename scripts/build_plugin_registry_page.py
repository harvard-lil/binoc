...
def _plugin_section(p: dict[str, Any], idx: int) -> list[str]:
    ...
    for ri, rule_pack in enumerate(rule_packs):
        ...
        rows.append(("`extensions`", ", ".join(f"`{x}`" for x in _as_str_list(exts, f'{rp}.dispatch.extensions')) if exts else "—"))
        rows.append(("`media_types`", ", ".join(f"`{x}`" for x in _as_str_list(mts, f'{rp}.dispatch.media_types')) if mts else "—"))
        rows.append(("`scope`", f"`{_as_str(dispatch.get('scope', 'files'), f'{rp}.dispatch.scope')}`"))
