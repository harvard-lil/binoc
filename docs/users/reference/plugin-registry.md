...
|---|---|
| Tier | `built-in` |
| Distribution | Built into every `binoc` install; no separate package is needed. |
...
| Distribution | Bundled in the default `binoc` 0.2.0 fat wheel. |
| Handles | Avro files. |
...
| Distribution | First-party Python add-on package discovered through `binoc.plugins` entry points. |
...
| Distribution | First-party opt-in rule pack. Separate PyPI publishing is paused, and SQLite is intentionally excluded from the default `binoc` 0.2.0 `bundled` feature set; build from source with the `sqlite` cargo feature when this pack is needed. |
...
|---|---|
| Tier | `first-party-bundled` |
| Distribution | Bundled in the default `binoc` 0.2.0 fat wheel. Separate PyPI publishing is paused. |
