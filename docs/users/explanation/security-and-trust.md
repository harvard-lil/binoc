...
archives expanded under a session workspace), runs correspondence rules and
renderers, and writes changelogs or changesets. The security posture
...
| Plugins (rule packs and renderers) | **Trusted code, same as host.** | Plugins run in-process. A malicious plugin can do anything the host process can. Binoc does not sandbox plugins. |
...
Standard library rule packs are part of the default attack surface: every user
who installs `binoc` runs them. Review them with the same rigor as any
third-party plugin.
