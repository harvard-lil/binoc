...
- *m* rule packs parse formats and propose correspondences.
- *n* compaction/annotation rules detect cross-cutting patterns.
- *o* renderer configs decide what each pattern means in your domain.

A new format costs one focused rule pack. A new analysis costs one focused
rule. A new domain opinion costs zero code — just a config edit. See
