...
The new move node represents the rename, but its content is still unknown. The fuzzy detector cannot synthesize a content diff itself: comparators are the parser, transformers are an optimization pass, and the IR enforces that boundary (see [transformer composition and artifact flow](2026-03-20-transformer_composition_and_artifact_flow.md)).
...
`pending_recompare` follows the wire-visible-but-stripped-at-session-exit pattern from [transient fields on wire](2026-04-16-transient_fields_on_wire.md): `#[serde(default, skip_serializing_if = "Option::is_none")]` for the ABI, cleared by `DiffNode::strip_transient` for user-facing output.
