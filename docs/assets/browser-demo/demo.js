...
function file(path, text) {
  return new File(encoder.encode(text), { readonly: true });
}
...
function insertPath(root, path, text) {
  ...
  dir.contents.set(parts[parts.length - 1], file(path, text));
}
...
function renderContent() {
  ...
  document.querySelector("[data-browser-demo-content]").textContent =
    demos[state.activeDemo].files[path] || "";
}
