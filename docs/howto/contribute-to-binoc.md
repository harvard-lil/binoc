...
[Diff two snapshots](diff-two-snapshots.md) instead.
...
- **The controller is type-ignorant.** Never add format knowledge to
  `binoc-core`. See [Architecture overview](../explanation/architecture.md).
...
Longer form: the [architecture overview](../explanation/architecture.md)
...
|---|---|
| Add a new test vector | [`test-vectors/`](https://github.com/harvard-lil/binoc/tree/main/test-vectors) — create `snapshot-a/`, `snapshot-b/`, `manifest.toml`. See [Test vectors explanation](../explanation/test-vectors.md). |
| Add a stdlib comparator or transformer | `binoc-stdlib/src/comparators/` or `.../transformers/`. For third-party plugins, see [Write a Python comparator](write-a-python-comparator.md) / [Write a Rust comparator](write-a-rust-comparator.md). |
| Change the IR | `binoc-sdk/src/ir.rs` (wire types) and `binoc-core/src/ir.rs` (in-memory additions). IR changes are high-leverage; read [IR and changesets](../explanation/ir-and-changesets.md) first. |
...
[Test vectors](../explanation/test-vectors.md).
...
- [Architecture overview](../explanation/architecture.md) — build
  the mental model before making a structural change.
- [Test vectors](../explanation/test-vectors.md) — the easy-first
  contribution path.
