...
**Prerequisites.** A Rust rule pack (see
[Write a Rust rule pack](write-a-rust-rule-pack.md)). The shared
...
use binoc_stdlib::test_vectors::{
    discover_vectors, run_vector_with_correspondence_engine_config,
    stdlib_materializers, VectorMaterializer,
};
...
fn test_vectors() {
    ...
    for vector in discover_vectors(vectors_root) {
        ...
        my_plugin::register_correspondence_rules(&mut config);
...
If your plugin is pure Python, it can currently author renderers and scripting
around `binoc.diff()`. For pytest coverage, invoke the public Python API and
assert the rendered output or changeset shape your renderer consumes:
...
- [Write a Rust rule pack](write-a-rust-rule-pack.md) — the
  rule pack you'll be testing here.
