...
| Add stdlib correspondence behavior | `binoc-stdlib/src/correspondence/` for expand, parse, pair, writer, compaction, and projection rules. For third-party plugins, start with [Plugin model](../../plugin-developers/explanation/plugin-model.md). |
...
| Fix the controller / dispatch logic | `binoc-core/src/controller.rs` and `binoc-core/src/correspondence/`. See [Dispatch model](../../plugin-developers/explanation/dispatch-model.md). |
...
(`csv-column-reorder`), not how (`test-csv-3`). See
