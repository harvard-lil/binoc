...
!!! note "Research note, not normative documentation"
    ...
    VII) to [Binoc: the architecture, told as a story](architecture-story.md),
...
- **[diffoscope](https://diffoscope.org/)** (Reproducible Builds; GPL-3,
  ...
  (depth 50), pluggable dispatch-by-type comparator classes, 100+ formats
  ...
  comparator output that isn't a transformable IR) should be ready.
...
- **[sqldiff](https://sqlite.org/sqldiff.html)** (SQLite project; public
  ...
  SQL — covers the SQLite comparator's core for nearly free.
- **Scientific formats**: **h5diff** (HDF5, tolerant object-by-object
  compare) and **nccmp**/NCO for NetCDF — single-format leaf-comparator
  prior art only.
- **Spreadsheets**: Microsoft **Spreadsheet Compare** (Office Pro
  Plus/M365; legacy, interactive only, no machine output);
...
reimplementations were necessary. The defensible novelty is Moves 3–8:
transformers, artifacts, recompare, extract, and the facts/judgments wall.
No incumbent has an IR you can transform.
...
- *Pass ordering:* MLIR deliberately has **no inter-pass dependency
  ...
  is the pipeline author's job. LLVM's legacy pass manager *had* declared
...
- *Fixpoint quarantine:* MLIR's `canonicalize` runs greedy rewriting to
  ...
  is ever needed, it becomes one bounded transformer with a convergence
...
- *Open vocabularies:* MLIR splits attributes into **inherent** (verified
  ...
  statement of which a transformer may drop on rewrite; a 2025 MLIR
  ...
  distinction — plus a `dependentDialects` analogue where transformers
...
- **Darcs/Pijul patch theory**: the maximalist position — changes as
  ...
  test for when transformer order *can't* matter.
...
renderers and transformers advertising which tag and artifact vocabularies
