//! Plugin test vectors: binoc-avro/test-vectors/. Uses the shared harness from
//! binoc_stdlib::test_vectors; the AvroMaterializer that builds `.avro` from
//! `.avro.d` lives in `binoc_avro::test_support`.
//!
...
use binoc_avro::test_support::AvroMaterializer;
        ...
        run_vector_with_correspondence_engine_config(
            ...
            |dataset| {
                ...
                if !is_without_plugin {
                    binoc_avro::register_correspondence_rules(&mut config);
                }
                config
            },
        );
